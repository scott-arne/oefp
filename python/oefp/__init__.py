...
def _user_cache_root():
    ...
    if cache_home:
        return Path(cache_home) / "oefp"
    return Path.home() / ".cache" / "oefp"
...
def _ensure_library_compat():
    ...
    When oefp is built with shared OpenEye libraries, the compiled extension
...
def _find_extension_module_path(pkg_dir):
    """Find the installed _oefp extension file."""
    for suffix in _extension_suffixes():
        candidate = Path(pkg_dir) / f"_oefp{suffix}"
        if candidate.is_file():
            return candidate
    for candidate in Path(pkg_dir).glob("_oefp*"):
...
def _load_cached_extension_if_needed():
    """Load _oefp from the cache when OpenEye aliases live there."""
    cache_dir = _OPENEYE_COMPAT_EXTENSION_DIR
    if cache_dir is None:
        return

    module_name = f"{__name__}._oefp"
    ...
    try:
        ...
        raise ImportError(
            f"Could not prepare cached oefp extension in {cache_dir}: {exc}"
        ) from exc
...
from .api import (
    ...
    FingerprintSpec,
    KallistoAtomDescriptors,
    Metric,
    MordredDescriptorSource,
    MorganGenerator,
    OEFP,
    OpenEyePropertyDescriptorSource,
    RDKitDescriptorSource,
    OEFPBatch,
    OEFPCount,
    OEFPCount64,
    OEFPCountBatch,
    ...
    TOPOLOGICAL_TORSIONS_PREREQUISITES,
    ...
    KallistoBondDescriptors,
    KallistoBondDescriptorsBatch,
    Sterimol,
    ...
    topological_torsions_count_fingerprint,
    topological_torsions_descriptors,
    topological_torsions_fingerprint,
    topological_torsions_sparse_count_fingerprint,
    topological_torsions_sparse_fingerprint,
)
