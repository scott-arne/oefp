...
OEFPCount64::OEFPCount64(FingerprintSpec spec)
    : spec_(validate_counted_spec(std::move(spec))) {
}

OEFPCount64::OEFPCount64(
    FingerprintSpec spec,
...
const FingerprintSpec& OEFPCount64::Spec() const {
    return spec_;
}

std::uint64_t OEFPCount64::SizeBits() const {
    return spec_.size_bits;
}

std::size_t OEFPCount64::NonzeroCount() const {
    return indices_.size();
}

std::uint64_t OEFPCount64::TotalCount() const {
    std::uint64_t total = 0;
    for (const auto count : counts_) {
        total += count;
    }
...
const std::vector<std::uint64_t>& OEFPCount64::Indices() const {
    return indices_;
}

const std::vector<std::uint32_t>& OEFPCount64::Counts() const {
    return counts_;
}

std::uint64_t OEFPCount64::Index(std::size_t row) const {
    CheckRow(row);
    return indices_[row];
}

std::uint32_t OEFPCount64::Count(std::size_t row) const {
    CheckRow(row);
    return counts_[row];
}

const std::uint64_t* OEFPCount64::IndexData() const {
    if (indices_.empty()) {
        return nullptr;
    }
    return indices_.data();
...
const std::uint32_t* OEFPCount64::CountData() const {
    if (counts_.empty()) {
        return nullptr;
    }
    return counts_.data();
...
std::uint64_t OEFPCount64::IndexDataAddress() const {
    const auto* data = IndexData();
    if (data == nullptr) {
        return 0;
    }
...
std::uint64_t OEFPCount64::CountDataAddress() const {
    const auto* data = CountData();
    if (data == nullptr) {
        return 0;
    }
...
void OEFPCount64::ValidateCountedSpec() const {
    if (spec_.value_type != FingerprintValueType::Counted) {
        throw std::invalid_argument("OEFPCount64 requires counted fingerprint metadata.");
    }
}

void OEFPCount64::ValidateSparseStorage() const {
    if (indices_.size() != counts_.size()) {
        throw std::invalid_argument("Count fingerprint indices and counts must have the same length.");
    }
    for (std::size_t row = 0; row < indices_.size(); ++row) {
...
void OEFPCount64::CheckRow(std::size_t row) const {
    if (row >= indices_.size()) {
        throw std::out_of_range("Count fingerprint row is out of range.");
    }
}
